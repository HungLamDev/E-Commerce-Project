    ...
    <div className="w-full text-base px-[10px] ">
      <Link to={`/${path.DETAIL_PRODUCT}/${productData?._id}/${encodedTitle}`}>
        <div className="w-full border p-[15px]">
          <div className="pb-6">
            {isNew ? (
              <span className="border px-4 pt-1 pb-1 rounded-[20px] text-yellow-500">
                New
              </span>
            ) : null}
          </div>
      ...
      <div className="flex pt-5 border-r border-l border-b">
        <div className="w-[60%] flex pt-1">
          {renderStarFromNunber(productData?.totalRatings)?.map((el, index) => (
            <div key={index}>{el}</div>
          ))}
        </div>
        <div className="w-[30%] flex pr-2 pb-1">Yêu thích</div>
